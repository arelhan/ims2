    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 mt-1 text-sm">Enter your username to get a reset code</p>
        </div>

        {resetCode ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-4">
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-center space-y-3">
              <p className="text-sm text-amber-800 font-medium">Your reset code (valid 15 min):</p>
              <div className="flex items-center justify-center gap-2">
                <span className="text-2xl font-mono font-bold tracking-widest text-slate-900">{resetCode}</span>
                <button onClick={copyCode} className="p-1.5 text-slate-400 hover:text-slate-700 transition">
            ...
            <Link href={`/reset-password?username=${encodeURIComponent(username)}`}
              className="block w-full bg-slate-900 text-white text-center rounded-xl py-2.5 text-sm font-medium hover:bg-slate-800 transition">
              Continue to Reset Password →
            </Link>
            <Link href="/login" className="block text-center text-sm text-slate-500 hover:text-slate-700 transition">
              Back to login
            </Link>
          </div>
        ) : (
