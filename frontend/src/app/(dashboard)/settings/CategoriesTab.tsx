...
export default function CategoriesTab() {
  ...
  return (
    <div className="max-w-lg space-y-4">
      {deleteError && (
        <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 rounded-xl px-4 py-3 text-sm">{deleteError}</div>
      )}
      ...
      {showAdd ? (
        <div className="flex gap-2">
          <input
            ...
            autoFocus
            className="flex-1 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900 dark:focus:ring-slate-400 dark:bg-slate-700 dark:text-slate-100"
          ...
          <button onClick={() => newName && createMutation.mutate()} disabled={!newName || createMutation.isPending}
            className="px-3 py-2 bg-slate-900 dark:bg-sky-600 text-white rounded-xl text-sm disabled:opacity-50 hover:bg-slate-800 dark:hover:bg-sky-500 transition">
            <Check size={16} />
          </button>
          <button onClick={() => { setShowAdd(false); setNewName('') }}
            className="px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-xl text-sm hover:bg-slate-50 dark:hover:bg-slate-700/50 transition">
            <X size={16} />
          </button>
        </div>
      ) : (
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        {(categories as any[]).length === 0 ? (
          <div className="p-10 text-center text-slate-400 dark:text-slate-500 text-sm">No categories yet</div>
        ) : (
