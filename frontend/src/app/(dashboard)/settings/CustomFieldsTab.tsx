  ...
  return (
    <div className="max-w-xl space-y-5">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        {categories.length === 0 && (
          <p className="text-sm text-slate-400">No categories yet. Add one in the Categories tab.</p>
        )}
      </div>
        ...
        <>
          ...
          {showAdd && (
            <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
              <h3 className="font-semibold text-slate-900 text-sm">New Custom Field</h3>

              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1.5 uppercase tracking-wide">Field Label *</label>
                <input
                  value={form.label}
                ...
                />
                {form.label && (
                  <p className="text-xs text-slate-400 mt-1">Key: <code className="bg-slate-100 px-1 rounded">{toFieldKey(form.label)}</code></p>
                )}
              </div>

              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1.5 uppercase tracking-wide">Field Type</label>
                ...
                {selectedFieldType && (
                  <p className="text-xs text-slate-400 mt-1.5">{selectedFieldType.desc}</p>
                )}
              </div>
              ...
              {form.fieldType === 'SELECT' ? (
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1.5 uppercase tracking-wide">Options *</label>
                  <input
                    ...
                    className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900"
                  />
                  <p className="text-xs text-slate-400 mt-1">Seçenekleri virgülle ayırın</p>
                  {form.selectOptions && (
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {form.selectOptions.split(',').map(o => o.trim()).filter(Boolean).map(opt => (
                        <span key={opt} className="px-2 py-0.5 bg-slate-100 text-slate-700 rounded-lg text-xs">{opt}</span>
                      ))}
                    </div>
                  )}
                </div>
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1.5 uppercase tracking-wide">Placeholder</label>
                  <input
              ...
              <label className="flex items-center gap-3 cursor-pointer select-none" onClick={() => setForm({ ...form, isRequired: !form.isRequired })}>
                <div className={`w-9 h-5 rounded-full transition-colors relative ${form.isRequired ? 'bg-slate-900' : 'bg-slate-200'}`}>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
            {(fields as any[]).length === 0 ? (
              <div className="p-10 text-center text-slate-400 text-sm">No fields yet for this category</div>
            ) : (
