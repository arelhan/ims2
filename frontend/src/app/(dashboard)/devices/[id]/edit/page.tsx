...
export default function EditDevicePage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="p-4 sm:p-8 max-w-2xl">
      <Link href={`/devices/${params.id}`} className="inline-flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 mb-6">
        <ArrowLeft size={16} /> Back
      </Link>

      <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-6">Edit Device</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {fields.length > 0 && (
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 sm:p-6 space-y-4">
            <h2 className="font-semibold text-slate-900 dark:text-slate-100">Specifications</h2>
