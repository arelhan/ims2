    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">{deviceRows.length} total devices</p>
        </div>
        <Link
          href="/devices/new"
          className="flex items-center justify-center gap-2 bg-slate-900 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-slate-800 transition sm:w-auto"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-slate-900"
          />
        </div>
        <select
          value={status}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center text-slate-400">No devices found</div>
        ) : (
          <>
            {/* Desktop table */}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-100">
                    ...
                    <th className="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Brand</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {deviceRows.map((device: any) => (
                    <tr key={device.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="px-6 py-3 text-sm text-slate-600">{device.category?.name}</td>
                      <td className="px-6 py-3 text-sm text-slate-600">{device.brand?.name || '—'}</td>
                      <td className="px-6 py-3">
                      ...
                      <td className="px-6 py-3 text-sm text-slate-400">{formatDate(device.createdAt)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="md:hidden divide-y divide-slate-100">
              {deviceRows.map((device: any) => (
                <Link
                  ...
                  className="flex items-start justify-between p-4 hover:bg-slate-50 transition gap-3"
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-slate-900 truncate">{device.name}</p>
                    <p className="text-xs font-mono text-slate-400 mt-0.5">{device.serialNumber}</p>
                    <p className="text-xs text-slate-500 mt-1">
                      {device.category?.name}{device.brand?.name ? ` · ${device.brand.name}` : ''}
                    </p>
                    {device.assignments?.[0]?.personnel?.name && (
                      <p className="text-xs text-slate-500 mt-0.5">→ {device.assignments[0].personnel.name}</p>
                    )}
                  </div>
                  <div className="shrink-0 flex flex-col items-end gap-1.5">
                    ...
                    <span className="text-xs text-slate-400">{formatDate(device.createdAt)}</span>
                  </div>
                </Link>
              ))}
            </div>
